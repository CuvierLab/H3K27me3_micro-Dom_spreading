...
int main (int argc, char **argv)
{
	...
	gamma = dmatrix(1, T, 1, hmm.N);
        ...
        ForwardWithScale(&hmm, T, O, alpha, scale, &logprobfinal);
        BackwardWithScale(&hmm, T, O, beta, scale, &logprobinit);
...
/* 	PrintHMM(stdout, &hmm); */
	...
	fprintf(stderr, "#Log Prob: %E\n", logprobfinal);

	for(i=1;i<=T;i++)
	  {
	    fprintf(stdout,"%5d",i);
	    for(j=1;j<=hmm.N;j++)
	      fprintf(stdout,"%12.8f", gamma[i][j]);
	    fprintf(stdout,"\n");
	  }
	
	/* print the posterior probs, gamma(i) */
	/*	for(i=1;i<=T;i++)
	  {
	    fprintf(stdout,"%5d",i);
	    for(j=1;j<=N;j++)
	      fprintf(stdout, "%8.4f", gamma[i][j]);
	    fprintf(stdout,"\n");
	  }
	*/
